...
/**
 * Process a QSKE payload during an IKE_AUX exchange
 */
static void process_qske_payload_aux(private_ike_init_t *this,
									 qske_payload_t *qske)
{
	qske_mechanism_t qske_mechanism;
...
static void process_payloads(private_ike_init_t *this, message_t *message)
{
	...
	{
		/* QSKE is either exchanged here or in a separate IKE_AUX exchange */
...
METHOD(task_t, build_i_aux, status_t,
...
METHOD(task_t, process_r_aux,  status_t,
	private_ike_init_t *this, message_t *message)
{
	qske_payload_t *qske;
	...
	if (message->get_exchange_type(message) == IKE_AUX)
	{
		...
		if (qske)
		{
			process_qske_payload_aux(this, qske);
		}
		else
		{
...
METHOD(task_t, build_r_aux, status_t,
	private_ike_init_t *this, message_t *message)
{
	qske_payload_t *qske;
	...
	private_ike_init_t *this, message_t *message)
{
	...
	if (!this->qske &&
		this->proposal->get_algorithm(this->proposal, QSKE_MECHANISM,
									  &alg, NULL))
	{	/* use IKE_AUX to exchange QSKE payloads, unless we are rekeying */
		if (this->old_sa)
		{
		...
		this->qske_mechanism = alg;
		this->public.task.build = _build_r_aux;
		this->public.task.process = _process_r_aux;
		return NEED_MORE;
	}
	return SUCCESS;
}
...
METHOD(task_t, process_i_aux, status_t,
	private_ike_init_t *this, message_t *message)
{
	qske_payload_t *qske;
	...
	private_ike_init_t *this, message_t *message)
{
	...
	if (!this->qske &&
		this->proposal->get_algorithm(this->proposal, QSKE_MECHANISM,
									  &alg, NULL))
	{	/* use IKE_AUX to exchange QSKE payloads */
		this->qske_mechanism = alg;
		this->public.task.build = _build_i_aux;
		this->public.task.process = _process_i_aux;
		return NEED_MORE;
	}
	return SUCCESS;
}
